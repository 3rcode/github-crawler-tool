...
def crawl_commits(owner, repo):
    ...
    commits = [repo.revparse_single(commit_sha).message for commit_sha in commit_shas]
    return commits

def get_commit(commit, replace=False):
    if not commit:
        print("Commit is none")
        return None
    html = markdown(commit)
...
def crawl_data():
    ...
    for i in range(num_repo):
        status = repos.loc[i, "Crawl status"]
        if status != "Done":

...
# Functions crawl data using github api

def github_api(owner, repo, type, func, option=""):
    """Get all specific component of element has type is type
    using github_api"""
...
def crawl_changelogs(owner, repo):
    return github_api(owner, repo, type="releases", 
                      func=lambda el: el["body"])


# def crawl_branches(owner, repo):
#     return github_api(owner, repo, type="branches", 
#                       func=lambda el: el["name"])
...
# Function crawl data using cli, pygit2
...
def crawl_commits(owner, repo):
    ...
    commits = [repo.revparse_single(commit_sha)
...
# Preprocessing crawled data functions

def get_commit(commit, owner, repo, sha, outlier_commit):
    """ Split commit into commit message (the first line) 
    and follow by commti description."""
    
    if not commit:
        outlier_commit["Empty commit"].append(
            {
                "message": commit,
                "owner": owner,
                "repo": repo,
    ...
    if not lines:
        outlier_commit["No p commit"].append(
            {
                "message": commit,
                "owner": owner,
                "repo": repo,
...
def crawl_data():   
    ...
    for i in range(num_repo):
        ...
        try:
            ...
            cs_arr = [get_changelog_sentences(changelog) 
                      for changelog in changelogs]  
            changelog_sentences = [sentence 
                                   for cs in cs_arr for sentence in cs]
            changelogs_df = pd.DataFrame({
                "Changelog Sentence": changelog_sentences
            })
            changelogs_df = changelogs_df.drop_duplicates(
                subset=["Changelog Sentence"], 
                keep="first"
            ).reset_index(drop=True)
            changelogs_df["Index"] = [idx + 1
                                      for idx in changelogs_df.index]
            changelogs_df = changelogs_df[["Index", 
                                           "Changelog Sentence"]]
            ...
            # commits = [
            #     get_commit(commit["message"], owner, repo, 
            #                commit["sha"], outlier_commit=outliers)  
            #     for commit in commits 
            #     if get_commit(commit["message"], owner, repo, 
            #                commit["sha"], outlier_commit=outliers)
            # ]
            ...
            # })
            # commits_df = commits_df.drop_duplicates(subset=["Message"])\
            #             .reset_index(drop=True)
            # commits_df["Index"] = [idx + 1
            #                        for idx in commits_df.index]
            # commits_df = commits_df[["Index", "Sha", "Owner", "Repo",
            #                          "Message", "Description"]]
